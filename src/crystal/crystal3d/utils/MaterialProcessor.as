package crystal.crystal3d.utils {
...
public class MaterialProcessor {
	 ...
	 */
	public function setupMaterials(objects:Vector.<Object3D>):void {
		...
		for (var objInd:uint = 0; objInd < objectsQuantity; objInd++) {
			...
			if (processedMesh != null) {
				...
				var meshHasNormals:Boolean = processedMesh.geometry && processedMesh.geometry.hasAttribute(VertexAttributes.NORMAL);
				var meshHashTangent:Boolean = processedMesh.geometry && processedMesh.geometry.hasAttribute(VertexAttributes.TANGENT4);
	...
	public function setupExternalTexture(texture:ExternalTextureResource, source:Object, isATF:Boolean):void {
		if (source) {
			...
			if (!isATF) {
				setupExternalTextureFromBitmap(texture, (source as Bitmap).bitmapData);
			}
